...
def move_frames(percentage, array_scores):
    ...
    no_of_frames = math.ceil(len(array_scores) * percentage / 100)

    no_of_frames_ratio = np.array_split(
        [i for i in range(0, no_of_frames)], no_of_sections)

    ind = []

    for idx, array_scores in enumerate(sections_array_scores):
        temp_no_of_frames = len(no_of_frames_ratio[idx])
        if temp_no_of_frames != 0:
            if len(array_scores) == 1:
                temp = [0]
            else:
                temp = np.argpartition(array_scores, temp_no_of_frames)[
                    :temp_no_of_frames]

            buffer = sum(len(x) for x in sections_array_scores[:idx])
